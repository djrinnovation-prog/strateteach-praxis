...
class BacktestConfig:
    ...
    slippage_ticks: int = 0           # DEPRECATED — never applied; see slippage_bps
    slippage_bps: float = 0.0         # market-fill slippage in basis points (1 bp = 0.01%)
    ...
    # Time stop (market exit at bar Open after N bars held; 0 = disabled). Only
    # honoured in the next-bar-open path (process_orders_on_close=False).
    long_max_bars: int = 0
    short_max_bars: int = 0
...
def _check_tpsl_fill(
    ...
    ``trail_sl_price`` (if > 0) is an absolute trailing-stop level supplied by the
    caller (computed from the running extreme since entry). It is merged into the
    stop as the *tighter* of the fixed stop and the trail (closer to price).
    ...
    # --- trailing stop merge: tighten the SL toward price ---
    # A trailing stop (driven by the running extreme since entry, supplied by the
    # caller as an absolute level) acts as an additional stop. The effective stop
    # is whichever is hit first intrabar = the one closer to price: the higher
    # level for a long, the lower level for a short.
    if trail_sl_price > 0:
        if sl_level is None:
            sl_level = trail_sl_price
        elif position_side == "long":
            sl_level = max(sl_level, trail_sl_price)
        else:
            sl_level = min(sl_level, trail_sl_price)
...
def apply_slippage(price: float, direction: str, slippage_bps: float) -> float:
    """Move a MARKET fill against the trader by *slippage_bps* basis points.

    Real fills are never free: a market buy lifts the offer, a market sell hits the
    bid. The engine used to declare `slippage_ticks` and never apply it, so every
    backtest silently assumed perfect fills — optimistic, and exactly the kind of
    thing that makes a backtest a poor guide to live behaviour.

    Expressed in BASIS POINTS rather than ticks on purpose: a "tick" needs a
    per-symbol minimum price increment we do not carry through the engine, whereas
    bps is well defined for every instrument and timeframe.

    Applies to MARKET fills only (entries, exits, stop-loss). A take-profit is a
    limit order — it fills at your price or better — so it is deliberately excluded.
    """
    if not slippage_bps:
        return price
    adj = 1.0 + (slippage_bps / 10_000.0) * (1 if direction == "buy" else -1)
    return price * adj


def _close_positions(open_positions, fill_price, bar_date, commission_rate,
                     trades, cash, side="long", slippage_bps: float = 0.0):
    """Settle every open sub-position at *fill_price*; append closed Trades to
    *trades* and return updated *cash*.

    Long PnL = qty*(fill-entry) and returns principal to cash; short PnL =
    qty*(entry-fill) and adjusts cash by PnL only (principal was never debited
    at short entry). Per-position arithmetic order is preserved exactly, so KPIs
    are byte-identical to the inlined loops this replaces. Covered by
    tests/test_engine_characterization.py.
    """
    # Closing a long SELLS (hits the bid); closing a short BUYS (lifts the offer).
    fill_price = apply_slippage(fill_price, "sell" if side == "long" else "buy", slippage_bps)
    for pos in open_positions:
        ...
        if side == "long":
            ...
            gross_pnl = pos.entry_qty * (pos.entry_price - fill_price)
...
def run_backtest(df: pd.DataFrame, config: BacktestConfig) -> dict:
    ...
    **Pyramiding** (``config.pyramiding > 1``):
    ...
    # Optional side-specific offsets (override the shared columns for that side)
    # and a trailing-stop offset. Long-only path only consults the long-side ones.
    has_long_sl_off = "long_sl_offset" in df.columns
    has_long_tp_off = "long_tp_offset" in df.columns
    has_long_trail_off = "long_trail_offset" in df.columns
    tp_sl_active = (config.take_profit_pct > 0 or config.stop_loss_pct > 0
                    or has_tp_col or has_sl_col or has_tp_off or has_sl_off
                    or has_long_sl_off or has_long_tp_off or has_long_trail_off
                    or config.long_max_bars > 0)
    # Running favourable extreme since entry (highest High for a long), used to
    # drive the trailing stop. Reset to the entry price at each entry fill.
    trail_extreme = 0.0
    ...
    for i in range(len(df)):
        ...
        if pending_exit and position_qty > 0:
            fill_price = bar["Open"]
            cash = _close_positions(open_positions, fill_price, bar_date, commission_rate, trades, cash, "long", slippage_bps=config.slippage_bps)
        ...
        if pending_entry:
            fill_price = apply_slippage(bar["Open"], "buy", config.slippage_bps)
            ...
            pending_entry = False
            entry_bar_idx = i
            trail_extreme = fill_price  # seed the trailing-stop extreme at entry
        ...
        if tp_sl_active and bar_in_range and position_qty > 0 and i > entry_bar_idx:
            ...
            # Side-specific offsets override the shared columns when present.
            bar_tp_off = bar["long_tp_offset"] if has_long_tp_off else (bar["tp_offset"] if has_tp_off else 0.0)
            bar_sl_off = bar["long_sl_offset"] if has_long_sl_off else (bar["sl_offset"] if has_sl_off else 0.0)
            # Trailing stop: level trails the running High since entry.
            trail_off = bar["long_trail_offset"] if has_long_trail_off else 0.0
            trail_sl_price = (trail_extreme - trail_off) if (trail_off > 0 and trail_extreme > 0) else 0.0
            # Time stop: after N bars held, force a market exit at this bar's Open.
            if config.long_max_bars > 0 and (i - entry_bar_idx) >= config.long_max_bars:
                bar_sl = bar["Open"]
        ...
        # 2d) Roll the trailing-stop extreme forward with this bar's High so the
        # next bar's trail level reflects all bars seen since entry (no lookahead).
        if position_qty > 0:
        ...
        if config.process_orders_on_close:
            ...
            if pending_entry and len(open_positions) < config.pyramiding:
                fill_price = apply_slippage(bar["Close"], "buy", config.slippage_bps)
                if pending_entry_qty > 0:
                    qty = pending_entry_qty
                    trade_value = qty * fill_price
                else:
    ...
    # Additive: expose per-bar equity curve for the dashboard detail view.
...
def run_backtest_long_short(df: pd.DataFrame, config: BacktestConfig) -> dict:
    ...
    **Position sizing** (``config.qty_type`` / ``config.qty_value``):
    ...
    has_tp_off = "tp_offset" in df.columns
    has_sl_off = "sl_offset" in df.columns
    # Side-specific offsets + trailing-stop offsets (selected by the open side).
    has_long_sl_off = "long_sl_offset" in df.columns
    has_short_sl_off = "short_sl_offset" in df.columns
    has_long_tp_off = "long_tp_offset" in df.columns
    has_short_tp_off = "short_tp_offset" in df.columns
    has_long_trail_off = "long_trail_offset" in df.columns
    has_short_trail_off = "short_trail_offset" in df.columns
    tp_sl_active = (config.take_profit_pct > 0 or config.stop_loss_pct > 0
                    or has_tp_col or has_sl_col or has_tp_off or has_sl_off
                    or has_long_sl_off or has_short_sl_off
                    or has_long_tp_off or has_short_tp_off
                    or has_long_trail_off or has_short_trail_off
                    or config.long_max_bars > 0 or config.short_max_bars > 0)
    # Running favourable extreme since entry (High for a long, Low for a short),
    # used to drive the trailing stop. Reset to the entry price at each entry.
    trail_extreme = 0.0
    ...
    for i in range(len(df)):
        ...
        if pending_short_exit and position_side == "short" and position_qty < 0:
            fill_price = apply_slippage(bar["Open"], "buy", config.slippage_bps)
            cash = _close_positions(open_positions, fill_price, bar_date, commission_rate, trades, cash, "short", slippage_bps=config.slippage_bps)
            equity = cash
            open_positions = []
            position_qty = 0.0
            position_entry_price = 0.0
        ...
        if pending_short_entry and (position_qty == 0 or (position_side == "short" and n_open < config.pyramiding)):
            fill_price = apply_slippage(bar["Open"], "sell", config.slippage_bps)
            if pending_entry_qty > 0:
        ...
        if tp_sl_active and bar_in_range and position_qty != 0 and i > entry_bar_idx:
            bar_tp = bar["tp_price"] if has_tp_col else 0.0
            bar_sl = bar["sl_price"] if has_sl_col else 0.0
            # Side-specific offsets / trailing / time stop selected by open side.
            if position_side == "long":
                bar_tp_off = bar["long_tp_offset"] if has_long_tp_off else (bar["tp_offset"] if has_tp_off else 0.0)
                bar_sl_off = bar["long_sl_offset"] if has_long_sl_off else (bar["sl_offset"] if has_sl_off else 0.0)
                trail_off = bar["long_trail_offset"] if has_long_trail_off else 0.0
                trail_sl_price = (trail_extreme - trail_off) if (trail_off > 0 and trail_extreme > 0) else 0.0
                max_bars = config.long_max_bars
            else:
                bar_tp_off = bar["short_tp_offset"] if has_short_tp_off else (bar["tp_offset"] if has_tp_off else 0.0)
                bar_sl_off = bar["short_sl_offset"] if has_short_sl_off else (bar["sl_offset"] if has_sl_off else 0.0)
                trail_off = bar["short_trail_offset"] if has_short_trail_off else 0.0
                trail_sl_price = (trail_extreme + trail_off) if (trail_off > 0 and trail_extreme > 0) else 0.0
                max_bars = config.short_max_bars
            # Time stop: after N bars held, force a market exit at this bar's Open.
            if max_bars > 0 and (i - entry_bar_idx) >= max_bars:
                bar_sl = bar["Open"]
            fill_price, fill_type = _check_tpsl_fill(
                bar_open=bar["Open"], bar_high=bar["High"], bar_low=bar["Low"],
            ...
            if fill_price is not None:
                ...
                if position_side == "long":
                    ...
                    cash = _close_positions(open_positions, fill_price, bar_date, commission_rate, trades, cash, "long")
                    equity = cash
                    ...
                    cash = _close_positions(open_positions, fill_price, bar_date, commission_rate, trades, cash, "short")
                    equity = cash
        ...
        # 2d) Roll the trailing-stop extreme forward with this bar (High for a
        # long, Low for a short) so the next bar's trail level reflects every bar
        # seen since entry (no lookahead within the current bar).
        if position_side == "long" and position_qty > 0:
            trail_extreme = max(trail_extreme, bar["High"])
        elif position_side == "short" and position_qty < 0:
        ...
        if config.process_orders_on_close:
            ...
            if pending_short_entry and (position_qty == 0 or (position_side == "short" and len(open_positions) < config.pyramiding)):
                fill_price = apply_slippage(bar["Close"], "sell", config.slippage_bps)
                if pending_entry_qty > 0:
                    abs_qty = pending_entry_qty
                    trade_value = abs_qty * fill_price
                else:
